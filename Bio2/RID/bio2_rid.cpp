...
std::uintmax_t Resident_Evil_2_RID::Open(StdFile& File, std::size_t nCut, std::uintmax_t _Ptr)
{
	...
	{
		Str.Message(L"Resident Evil 2 Error: could not read RID at 0x%llX in \"%ws\"", _Ptr, File.GetPath().filename().wstring().c_str());
		return _Ptr;
	}
...
std::uintmax_t Resident_Evil_2_RID::Save(StdFile& File, std::uintmax_t _Ptr)
{
	...
	{
		Str.Message(L"Resident Evil 2 Error: could not write RID at 0x%llX in \"%ws\"", _Ptr, File.GetPath().filename().wstring().c_str());
		return _Ptr;
	}
