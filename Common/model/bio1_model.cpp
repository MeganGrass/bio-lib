...
/*
	Set game
*/
void Resident_Evil_Model::SetGame(Video_Game _Game)
{
	Game = _Game;
...
bool Resident_Evil_Model::Open(std::filesystem::path Input)
{
	...
	else if (std::to_underlying(Game) & (AUG95 | OCT95))
	{
		...
		if (!Tim->OpenTIM(File, Offset[2]) || Tim->Size() != (File.Size() - (Offset.size() * sizeof(std::uint32_t)))) { return false; }
	}
	else
	{
		if (Offset[0])
		...
		if (!Tim->OpenTIM(File, Offset[4]) || Tim->Size() != (File.Size() - (Offset.size() * sizeof(std::uint32_t)))) { return false; }
	}
...
/*
	Extract
*/
bool Resident_Evil_Model::Extract(std::filesystem::path Input, Video_Game _Game)
{
	Standard_String Str;
	...
	if (Emd->Tim->IsOpen())
	{
		...
		Text->AddLine("%s\n", Filename.filename().string().c_str());
		Emd->Tim->SaveTIM(Filename);

		// BMP
		std::unique_ptr<Standard_Image> Image = Emd->Tim->ExportImage();
		for (uint32_t i = 0; i < Emd->Tim->GetPaletteCount(); i++)
		{
			Filename = Str.FormatCStyle("%s\\%s\\%s_%02d.bmp", Dir.string().c_str(), Input.stem().string().c_str(), Input.stem().string().c_str(), i);
			Emd->Tim->UpdateImagePalette(Image, i);
